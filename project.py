...
def gconnect():
    ...
    # get user_id of the user currently logged in

    user_id = getUserID(login_session['email'])

    # check if user_id is not equal to the login_session's user_id.
    # if true, then create new user with login_session credentials,
    # and equate user_id in login session to current User's id.a
...
def gdisconnect():
    ...
    if result['status'] == '200':
        ...
        response = \
            make_response(json.dumps('Failed to revoke token for user'
                          ), 400)
        response.headers['Content-Type'] = 'application/json'
        return response
